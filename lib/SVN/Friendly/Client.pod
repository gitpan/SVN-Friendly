...
nearly raw API exposed by the Perl subversion bindings provided
by L<SVN::Client|>.  Like those bindings, it provides full access
to the functions of the raw API but does so in a way that integrates
naturally with Perl programming styles and language features.
...
=item B<encapsulating domain knowledge>
...
=item B<parameter defaults and optional trailing parameters>
...
=item B<provide log comments via method parameters>
...
=item B<consistent easy-to-remember parameter ordering>
...
=item B<consistant naming of methods belonging to later APIs>
...
=item B<string representation of Subversion constants>
...
=item B<simplified access to repository and working copy metadata>

L<SVN::Friendly::Client|SVN::Friendly::Client> provides several
C<getXXX> methods to access commonly needed informtion about
repositories, working copies, and the paths within them.
...
=item B<automatic parameter type conversions>
...
=item B<simplified configuration of the authentication process>
...
=item B<simplified configuration of notifications>
...
Due to the way L<SVN::Client|> presently (1.6.12 and
earlier)automates the conversion of the raw C-API to object methods,
L<SVN::Client|> cannot be subclassed.  Even though the
...
created via the C<configureLogMsg> method.

You can also use the raw subversion API to change the log message.However
this is not recommended for two reasons.  First, you will lose the ability
to pass commit messages via method calss.  This is implemented by wrapping
your custom log_msg routine with a closure that populates the log message
from the parameter list and then calls your custom routine.  A second
reason is that the SWIG bindings between versions 1.4 and 1.5 broke
backwards compatibility and the C<log_msg_baton> method from the 1.4 API
causes parameter type errors in the 1.5 bindings.  C<configureLogMsg>
insures that the right method is called for the right subversion release.
...
file contents and properties to the central repository.  C<commit>
always returns C<svn_client_commit_info_t> if a commit occurs and
C<undef> otherwise.

The behavior of the major_minor version methods depends on the release.

* C<commit1_1> returns C<svn_client_commit_info_t> if
  a commit occurs and C<undef> otherwise. B<Note:> in the raw subversion
  1.5 bindings a failed commit returns C<svn_client_commit_info_t> in
  lieu of C<undef>. This has the potential to break programs that were
  originally written to use the 1.4 bindings or earlier.
  C<SVN::Friendly> wishes to avoid breaking software so this method
  I<always> returns C<undef> even when running with Perl bindings
  generated for subversion 1.5

* 1.4, 1.5 return C<svn_commit_info_t> in both cases of failure and
  success.
...
C<svn_commit_info_t> has two additional fields, one describing the nature
of any error and another identifying the repository that recieved the commits.
...
Between 1.4 and 1.5 this method changed behavior. All methods were affected
including those belonging to the 1.1. and 1.4 API. The changes included

=over

=item *

In 1.4 attempts to make a copy of a copy failed, if the
first copy wasn't committed.  Starting in 1.5 you could make a copy
of a copy before committing the first copy.

=item *

Starting with subversion 1.5 copied files and directories automatically get
C<svn:mergeinfo> property.

=item *

Action notification changed to look more like update than add. In 1.4 there
was an action notification for each file or directory added.  In 1.5 there
appears to be one update for each changed directory and its parents.
...
In 1.4 attempts to move a moved dir failed, if the
first move wasn't committed.  Starting in 1.5 you can move a move
before committing the first copy.

=item *

Starting with subversion 1.5 moved files and directories automatically get
C<svn:mergeinfo> property.
...
C<commit1_4> - works, but returns an unusable object. The object's
  package does not appear to have been imported into Perl, This bug
  appears to have persisted in the Perl bindings released with
  subversion 1.5.
